...
exports.handler = async (event, context) => {
  ...
  const securityResult = await securityMiddleware(event, context, 'video-comments');
  if (securityResult) return securityResult;

  const { videoId } = event.queryStringParameters || {};
  
  // Validate video ID
  const validation = validateRequiredParam({ videoId }, 'videoId', VALIDATORS.youtubeVideoId);
  if (!validation.valid) {
